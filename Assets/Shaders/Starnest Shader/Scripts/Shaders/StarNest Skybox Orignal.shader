...
Shader "All4thlulz/StarNest/Skybox/Original"
{
	Properties
	{
		...
		//Shift in hue
		_HueShift ("Hue Shift", Range(0, 1000)) = 0
		...
		_Center ("Center Position (x, y, z, time)", Vector) = (0, 0, 0.0, 0)

		//How much does camera position cause the effect to scroll?
		//Can give a really cool depth effect as you move the camera
		_CamScroll ("Camera Scroll", Range(-1000,1000)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f IN) : SV_Target
			{
				...
				float4 vertColor = float4(toRGB(hsv),1.0);

				return float4(vertColor.rgb * .01,1.0);
			}
			ENDCG
		}
	}
	...
	Fallback Off
	CustomEditor "All4thlulz.Editor.StarNestSkyboxOriginalInspector"
}
