...
Shader "All4thlulz/StarNest/Skybox/2D (ViewDir)" {
	Properties
	{
		...
		_PostSaturation ("Saturation Shift", Range(-1,1)) = 0
		...
		// z not active due to uv fix
		[HideInInspector]_Center ("Center Position (x, y, z, time)", Vector) = (0, 0, 0.5, 0)

		//This is now being used to correct the Z axis UV stretching that occurs
		[HideInInspector]_UVStretchFixValue("",float) = 0.5
		...
		//Brightness scale.
		_Brightness ("Brightness", Range(0,50)) = 2
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			float3 from = float3(_Center.x, _Center.y,_UVStretchFixValue * IN.viewDir.z);
			...
			// somewhat of a post processing effect to get hue shifting
			float3 hsv = toHSV(v);
			hsv.r += _HueShift;
			hsv.g += _PostSat;

			fixed4 vertColor = float4(toRGB(hsv),1.0);

			o.Emission = float3(vertColor.rgb * .01);
		}
		ENDCG
	}

	Fallback Off
	CustomEditor "All4thlulz.Editor.StarNestSkyboxInspector"
}
